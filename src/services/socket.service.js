...
//                     beginDate: moment(new Date()).format('h:mm:ss \n DD/MM/YYYY ') + "",
...
const toggleAudio = ({ dispatch, getState }) => next => action => {
    if (action.type === 'TOGGLE_AUDIO_STATUS') {
        dispatch(actions.setMicrophoneStatus(action.payload));
        window.store.getState().socketReducer.localStream.getAudioTracks()[0].enabled = action.payload;
...
// function toggleAudio(audioStatus) {
//     window.store.getState().socketReducer.localStream.getAudioTracks()[0].enabled = audioStatus;
...
const toggleVideo = ({ dispatch, getState }) => next => action => {
    debugger
    if (action.type === 'TOGGLE_VIDEO_STATUS') {
        dispatch(actions.setCameraStatus(action.payload));
        window.store.getState().socketReducer.localStream.getVideoTracks()[0].enabled = action.payload;
...
// function toggleVideo(videoStatus) {
...
//     //     .enabled;
//     window.store.getState().socketReducer.localStream.getVideoTracks()[0].enabled = videoStatus;

//     window.store.getState().conversationReducer.cameraStatus = videoStatus;
...
//     record.name = state.conversationReducer.roomId + "_" + Date.now().toString() + ".webm";
